...
%% at the appropriate time, display the go signal
    ...
    if td.direction == 'left' %#ok<STCMP>
        draw_arrow(w, [exp.display.x_center exp.display.y_center], 0, exp.display.colors.green, [30 80 75 30])
    else
        draw_arrow(w, [exp.display.x_center exp.display.y_center], 180, exp.display.colors.green, [30 80 75 30])
    end
...
%                     elseif isnan(right_rt) && key_list(exp.keyboard.right_key), right_rt = press_time; right_has_responded = true; 
            ...
            if stop_signal_pending && GetSecs > timing.stop_signal
                ...
                if td.direction == 'left' %#ok<STCMP>
                    draw_arrow(w, [exp.display.x_center exp.display.y_center], 0, exp.display.colors.red, [30 80 75 30])
                else
                    draw_arrow(w, [exp.display.x_center exp.display.y_center], 180, exp.display.colors.red, [30 80 75 30])
                end
